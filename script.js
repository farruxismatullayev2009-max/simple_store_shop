  ...
  document.body.insertAdjacentHTML(
    ...
    <style>
      #cat-select {
        ...
        padding:13px 16px; font-size:15px; color:#e2e8f0;
    ...
    ">
      <div id="filter-sidebar" style="
        background:#060e1c; width:100%; max-width:400px;
        height:100%; overflow-y:auto; padding:28px 24px;
        display:flex; flex-direction:column; gap:20px;
  ...
  function renderProducts(list, query = '') {
    ...
    document.querySelectorAll('.card[data-id]').forEach(card => {
      ...
      likeI.addEventListener('click', e => {
        ...
        likeCount += liked ? -1 : 1;
        updateBadge(heartBadge, likeCount);
        if (!liked) showToast('heart', title);
      });

      // Cart icon (hover da chiqadigan)
      card.querySelector('.btn-cart-icon').addEventListener('click', e => {
        e.stopPropagation();
        cartCount++;
        updateBadge(cartBadge, cartCount);
